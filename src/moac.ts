...
export default class Moac {
    ...
    public async getBalance(address: string): Promise<number> {
        ...
        try {
            ...
            balance = parseFloat(bnBalance.dividedBy(10 ** 18).toString(10));
        } catch (error) {
            balance = 0;
        }
        return balance;
     ...
     * @param {string} value generated from amount by bignumber.js. e.g: if the amount is 1 and the decimals of token is 2, the value is "100"
    ...
    public getTx(from: string, to: string, nonce: number, gasLimit: number, gasPrice: string, value: string, calldata: string): IMoacTransaction {
        ...
        const tx = {
            ...
            to,
            value: this._chain3.intToHex(value)
        };
        return tx;
    }
